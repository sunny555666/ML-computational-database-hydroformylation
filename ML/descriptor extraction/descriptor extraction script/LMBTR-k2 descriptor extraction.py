...
lmbtr_params = {
    ...
    'geometry': {'function': 'inverse_distance'},  # Geometry function
    'grid': {'min': 0, 'max': 5, 'n': 100, 'sigma': 0.1},  # Grid parameters
    ...
    if filename.endswith('.xyz'):
        ...
        for i_species in lmbtr_params['species']:
            for j_species in lmbtr_params['species']:
                if i_species == 'Rh' or j_species == 'Rh':
                    try:
                        # Ensure the center species is valid
                        if i_species == 'Rh':
                            loc = lmbtr.get_location(('X', j_species))
                            col_name = f"Rh-{j_species}"
                        elif j_species == 'Rh':
                            loc = lmbtr.get_location((i_species, 'X'))
                            col_name = f"{i_species}-Rh"
                        ...
                        print(f"Failed to plot {i_species}-{j_species} for {filename}: {e}")
                        continue
        ...
        if plotted:
            ...
            csv_filename = os.path.join(target_folder, f"{base_name}_lmbtrk2.csv")
            df.to_csv(csv_filename, index=False)
            print(f"LMBTR plot and data saved for {filename} (plot: {base_name}_lmbtr.png, data: {base_name}_lmbtrk2.csv)\n")
        else:
