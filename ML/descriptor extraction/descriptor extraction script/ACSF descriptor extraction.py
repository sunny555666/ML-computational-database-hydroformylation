...
# ACSF descriptor parameters (modify according to your chemical system)
acsf_params = {
    'species': ['Rh', 'C', 'O', 'P', 'H'],  # Add/modify elements as needed
    'r_cut': 6.0,           # Cutoff radius
    'g2_params': [[5,4],[2.5,3],[1,2],[0.4,1.5],[0.2,1],[0.1,0.5]],
    'g3_params': [0.5,1,1.5,2,2.5],
    'g4_params': [[5,1,-1],[5,1,1],[2.5,1,-1],[2.5,1,1],[1,1,-1],[1,1,1],
                 [5,2,-1],[5,2,1],[2.5,2,-1],[2.5,2,1],[1,2,-1],[1,2,1],
                 [5,3,-1],[5,3,1],[2.5,3,-1],[2.5,3,1],[1,3,-1],[1,3,1]]
}

# Create ACSF descriptor object
...
def main():
    ...
    for filename in os.listdir(INPUT_FOLDER):
        if filename.endswith(FILE_EXTENSION):
            ...
            # Calculate ACSF descriptors
            try:
                acsf_descriptors = acsf.create(selected_atoms)
                
                print(f"ACSF descriptors calculated:")
                print(f"Shape: {acsf_descriptors.shape}")
                
                # Save ACSF descriptors
                npy_filename = f"{base_name}{OUTPUT_SUFFIX}_acsf.npy"
                npy_path = os.path.join(INPUT_FOLDER, npy_filename)
                np.save(npy_path, acsf_descriptors)
                
                csv_filename = f"{base_name}{OUTPUT_SUFFIX}_acsf.csv"
                csv_path = os.path.join(INPUT_FOLDER, csv_filename)
                np.savetxt(csv_path, acsf_descriptors, delimiter=',', fmt='%s')
                
                print(f"ACSF descriptors saved to {csv_path}")
                
            except Exception as e:
                print(f"Failed to calculate ACSF descriptors for {filename}: {e}")
